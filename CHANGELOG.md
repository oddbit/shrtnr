...
- **The Worker applies its own migrations.** `migrations/*.sql` is generated into `src/db/migrations.generated.ts` and ships inside the bundle, because Workers have no file system to read SQL from. On the first request an isolate receives, the Worker applies every migration missing from D1's `d1_migrations` table, the same table with the same file names that `wrangler d1 migrations apply` records, so the CLI and the Worker can each go first and the other sees the work. Each migration lands as one batch with its bookkeeping row, so two isolates racing on a cold deploy still apply it once. A KV read of the recorded version short-circuits the D1 round trip for a fresh isolate, and a warm one awaits a settled promise, so redirects pay nothing.
- **`wrangler.jsonc` declares bindings by name and wrangler provisions them.** The KV placeholder id and `scripts/resolve-bindings.sh` are gone. The first `yarn deploy` creates the namespace and the database, later deploys link to them by binding name, and the Workers Builds container no longer makes a D1 list call that its token may not be allowed to make.
- **`/_/health` and `/_/setup` report on the schema, and neither changes it on a GET.** Health returns `schema.version`, `schema.applied` and `schema.ready`; a database no request has reached answers `ready: false` with 200, and a remembered failure answers 503 with the error. `GET /_/setup` lists applied and pending migrations, `POST /_/setup` retries at once, and both require a verified Access identity once `ACCESS_AUD` is set.
- **A failing migration fails open for visitors.** With an older schema in place, short links keep redirecting, since the redirect reads tables that already exist, while the admin pages, the API and the MCP transport answer a 503 that names the failing migration and quotes the database error. A fresh deploy with no schema still answers 503 everywhere, because there is nothing to serve. A failure is remembered for 30 seconds, so a migration that fails against live data costs one statement batch per half minute rather than one per request.
- **MCP tool descriptions name the neighbouring tool.** Each description states whether the action is reversible and points at the alternative when clicks exist, so an assistant picks `disable_link` over `delete_link` for a link with history and `archive_bundle` over `delete_bundle` for a finished campaign. `add_custom_slug` says that one link can carry several slugs, `create_bundle` says what a bundle rolls up, and the analytics descriptions name the dimensions they return. Descriptions only; no handler changed.
- **The README leads with the MCP server, bundles, multiple slugs per link and ownership**, then the table stakes, then a line naming what the project does not have yet. The Cloudflare Access and MCP walkthroughs move to `docs/access-control.md` and `docs/mcp.md`, each left in the README as one paragraph and a link, so the project stops looking harder to run than it is. `docs/access-control.md` carries a permission table derived from the service layer and the ownership suite. Screenshots render from `docs/images/` instead of the website, and a recording of Claude creating links over MCP sits above the fold as an animated WebP, 531 KB against the 8.5 MB GIF it replaces at the same resolution and duration.
- **Only the app track creates a GitHub Release, and it claims the Latest slot.** A Python client patch had taken the repo's most visible version signal. The npm, PyPI, pub.dev and extension workflows tag and publish exactly as before without calling `gh release create`. The app workflow fails loudly if the resolved tag is not `app-v<semver>`, passes `--latest`, and titles the release `v<version>`.
- **The repo states how to report a vulnerability and how to open a pull request.** `SECURITY.md`, `CONTRIBUTING.md` and the issue templates land alongside the existing license, notice and trademark files.
- The over-cap slug fixtures queue their rows in one batch instead of one `addCustom()` call each, which drops the failing breakdown test from 394ms to 24ms and removes a 5s timeout that only CI timing triggered. No assertion changed.
- OpenAPI paths and schemas are unchanged from 0.40.0; only `info.version` moves. The bump refreshes the recorded spec hash in all three SDKs, which ship no code change in this release.

## 0.40.0 (2026-09-22)

Performance, accessibility and defect release (PRs #59, #60 and #64, plus the post-merge review of #64). A cold admin load drops from 4.1 MB to 419 KB, a warm navigation moves 3 KB, and layout shift is zero. The release also closes a URL-normalization corruption on the redirect path, an SSRF pattern in the title fetcher, and unscannable QR codes.

- **The icon font is 322 KB instead of 3.98 MB.** The layout requested Material Symbols as a variable font with four axis ranges, 97% of the cold page weight, while the stylesheet only ever renders one weight and no fill. The static instance keeps every glyph, which matters because bundle icons are free text.
- **Fonts are self-hosted with metric-matched fallbacks, and nothing shifts when they arrive.** Manrope, Space Grotesk and Material Symbols live under `public/fonts` with the upstream version in the file name, licenses beside them and immutable caching; the two render-blocking Google Fonts stylesheets and their preconnects are gone. Icon spans hold a fixed 1em box, and local fallback faces with `size-adjust` match the web fonts' metrics, so CLS on the link detail page goes from 0.106 to 0. Preloads were measured and rejected: on Fast 3G they compete with the stylesheet and delay first paint.
- **The admin stylesheet and client script are content-hashed assets.** Every admin document inlined 75 KB of CSS and 90 KB of JS and shipped no-cache. `/_/assets/admin.<hash>.css` and `/_/assets/client.<lang>.<hash>.js` cache as immutable and sit outside the admin auth, since they carry nothing account-specific. During a deploy window a document from one build can request its assets from an isolate on another; the route recognises the asset the path names and serves its own copy with `no-store` rather than 404. The document decodes to 11 KB, down from 170 KB.
- **Static assets carry cache headers.** `public/_headers` caches the version-stamped `htmx-2.0.4.min.js` and the fonts as immutable, and brand assets for a day with stale-while-revalidate.
- **Screen readers get a usable admin DOM.** Every icon ligature is `aria-hidden`, each page has one `h1`, settings labels use `for=`, the quick URL box, the link detail editors, the back arrow and the inline save and cancel buttons carry names, and the links table declares column headers.
- **The settings page states whether MCP OAuth is configured.** The status only reached the document by riding inside the inline client script's translation table. The MCP card now renders it.
- **Unhandled exceptions log one JSON object and answer a body the client can parse.** Hono's default printed unstructured text. `src/unhandled.ts` logs method, path, error and stack as one Workers Logs entry. The route groups that answer JSON (`/_/api`, `/_/admin/api`, `/_/admin/w`, the MCP transport) declare it where they mount; everything else answers plain text unless the client accepts JSON.
- **JWKS verification no longer awaits another request's fetch.** A module-level jose resolver shared its in-flight fetch between concurrent first verifications in a fresh isolate, which Workers reject with "Cannot perform I/O on behalf of a different request". The resolver is per request over a shared key cache, so certificates still download once per isolate.
- **`normalizeUrl` keeps a trailing `?` or `#` that is URL content.** It stripped the last character whenever it was `?` or `#`, whether or not that character was the delimiter, so `https://a.com/?x=y?` and an embedded redirect target ending in `/?` were corrupted before storage and on every 301.
- **The title fetcher only fetches public hosts.** `autoLabelLink()` fetched any user-supplied URL server-side and followed redirects blind. `isPublicHttpUrl()` rejects loopback, RFC 1918, link-local, cloud-metadata and every reserved range, including IPv4-mapped and NAT64 literals and trailing-dot hosts. Redirects are followed by hand with each hop vetted, and the whole chain shares one 5 s deadline instead of one per hop.
- **QR codes for versions 6 to 10 scan.** Payloads over 106 bytes produced symbols no reader decoded: the encoder ran one Reed-Solomon pass where the spec requires split and interleaved blocks, never wrote the version-information blocks versions 7 and up require, and let alignment patterns overlap the finders. Verified with a real decoder. Payloads encode as UTF-8 and size the version by byte length, and the GF(256) tables are built once.
- **Settings setters report failure.** `setTheme()` and `setLanguage()` toasted success and reloaded whether or not the PUT succeeded, so a transient 5xx or an expired session looked like a saved preference. Both check `res.ok` like their siblings.
- **The add-slug modal stays open when the slug is rejected**, and the client-side `t()` treats interpolated values as literal text like the server does, so a key title such as `Prod $$ key` no longer renders as `Prod $ key`.
- **`SlugRepository.disable()` throws on a non-custom slug** instead of returning null and leaving the link without a primary. The unreachable New Link modal and the dead code behind it are removed.
- **The vitest suite runs off the network.** Every created link scheduled a real title fetch in `waitUntil` and nothing stubbed it; a miniflare outbound stub answers those fetches, which removes a CI timeout and a cross-test foreign-key error.
- `compatibility_date` advances to 2026-08-22, the newest date the vitest pool's bundled workerd accepts. `yarn types` generates `worker-configuration.d.ts` from `wrangler.jsonc` in place of the hand-written `Env`. `e2e/perf.spec.ts` pins page-weight budgets, layout shift and DOM accessibility invariants inside `yarn e2e`, and `yarn perf` runs it under Fast 3G with 4x CPU throttling.
- OpenAPI paths and schemas are unchanged from 0.39.0; only `info.version` changes. The bump refreshes the recorded spec hash in all three SDKs. TypeScript ships 1.2.1, Python 1.2.1 and Dart 2.2.1 alongside it, each tightening the transport's response-shape checks. Full suite: 93 files, 1421 tests.
